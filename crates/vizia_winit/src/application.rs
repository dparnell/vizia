...
impl Application {
    ...
    fn create_window(
        ...
        let window_attributes = apply_window_description(window_description);
...
impl ApplicationHandler<UserEvent> for Application {
    ...
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        let window = self
            .create_window(event_loop, Entity::root(), &self.window_description.clone())
            .expect("failed to create initial window");
        self.cx.add_main_window(Entity::root(), &self.window_description, 1.0);
        self.cx.add_window(Window { window: Some(window) });
        ...
        //     }
        //     let window = self
        //         .create_window(event_loop, window_entity, &window_state.window_description)
    ...
    fn about_to_wait(&mut self, event_loop: &ActiveEventLoop) {
        ...
        for window in self.cx.0.windows.values() {
            if self.windows.contains_key(unsafe { &WindowId::dummy() }) {
                self.windows.remove(unsafe { &WindowId::dummy() });
            }
        }
